...
// - Member → only projects they have tasks on
// ===============================
const getProjects = asyncHandler(async (req, res) => {
  let query = {};
...
// ===============================
// ⭐ GET PROJECT BY ID
// - Admin/Manager → access any project
...
const getProjectById = asyncHandler(async (req, res) => {
  ...
  if (!["Admin", "Manager"].includes(req.user.role)) {
    const userTasks = await Task.find({ assignedTo: req.user._id, projectId: project._id });
...
// ===============================
// ⭐ PROJECT ANALYTICS
// - Admin/Manager can view analytics for any project
// - Members only if they belong to that project
// ===============================
const getProjectAnalytics = asyncHandler(async (req, res) => {
  const project = await Project.findById(req.params.id);
  ...
  const statusCount = { todo: 0, progress: 0, done: 0 };
  const userWorkload = {};
